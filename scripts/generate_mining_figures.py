...
Produces matplotlib PNGs into assets/ for README Illustrated Outputs.
"""
import sys, os, math
...
# ========================================================================
# FIGURE 1: Rock Mass — GSI vs σ_cm for varying mi
# ========================================================================
print("[1] rock_mass_gsi_strength.png")
fig, ax = plt.subplots(figsize=(8,5))
gsi_range = list(range(10, 101, 5))
for mi, color in [(15, "#e94560"), (25, "#0f3460"), (35, "#533483")]:
    strengths = []
    for gsi in gsi_range:
        p = rm.hoek_brown_parameters(gsi, mi, 0.3)
        strengths.append(p['sigma_cm_MPa'])
...
# ========================================================================
# FIGURE 2: Ventilation — Psychrometric state (T vs RH)
# ========================================================================
print("[2] ventilation_psychrometric.png")
fig, ax = plt.subplots(figsize=(8,5))
...
ax.set_title("Exercise 2: Mine Air Psychrometric Enthalpy vs Temperature")
ax.legend()
ax.grid(True)
save(fig, "02_ventilation_psychrometric")

# ========================================================================
# FIGURE 3: Slurry — Density vs Solids Concentration
# ========================================================================
print("[3] slurry_density_concentration.png")
fig, ax = plt.subplots(figsize=(8,5))
Cw_range = [i/100 for i in range(0, 71, 5)]
rhos = [2800, 4200, 7800]  # copper, iron, gold ore
labels = ["Copper (2800)", "Iron (4200)", "Gold (7800)"]
colors = ["#e94560", "#0f3460", "#533483"]
for rho_s, label, color in zip(rhos, labels, colors):
...
ax.set_xlabel("Solids Concentration Cw (% by weight)")
ax.set_ylabel("Slurry Density (kg/m³)")
...
# ========================================================================
# FIGURE 4: Blast — PPV vs Distance for various charge weights
# ========================================================================
print("[4] blast_vibration_distance.png")
fig, ax = plt.subplots(figsize=(8,5))
...
ax.set_ylabel("Peak Particle Velocity (mm/s)")
...
# ========================================================================
# FIGURE 5: Integrated — Multi-panel Dashboard
# ========================================================================
print("[5] integrated_dashboard.png")
...
ax.set_xlabel("GSI")
ax.set_ylabel("E_rm (MPa)")
ax.set_title("Rock Mass Modulus")
ax.grid(True)
...
ax.set_xlabel("Dry T (C)")
ax.set_ylabel("Wet Bulb (C)")
ax.set_title("Psychrometric")
ax.legend()
ax.grid(True)
...
ax.set_title("Slurry Viscosity")
ax.grid(True)

# Dewatering: NPSH vs depth
ax = axes[1,0]
...
ax.set_xlabel("Sump Depth (m)")
ax.set_ylabel("NPSH Available (m)")
ax.set_title("Dewatering NPSH")
ax.grid(True)
...
ax.set_xlabel("Pore Pressure Ratio ru (%)")
ax.set_ylabel("Factor of Safety")
ax.set_title("Slope Stability vs Groundwater")
ax.grid(True)
...
ax.set_title("Blast Vibration")
ax.set_yscale("log")
ax.grid(True)

fig.suptitle("Exercise 5: Integrated Mine Design Dashboard", fontsize=14, fontweight="bold")
fig.tight_layout()
save(fig, "05_integrated_dashboard")

# ========================================================================
# FIGURE 6: Groundwater — Inflow vs Drawdown
# ========================================================================
print("[6] groundwater_inflow_drawdown.png")
fig, ax = plt.subplots(figsize=(8,5))
...
ax.set_xlabel("Drawdown (m)")
ax.set_ylabel("Steady-State Inflow (m³/h)")
ax.set_title("Exercise 6: Groundwater Inflow vs Drawdown")
ax.grid(True)
save(fig, "06_groundwater_inflow_drawdown")

# ========================================================================
# FIGURE 7: Rock Comparison — Bar chart
# ========================================================================
print("[7] rock_comparison_bar.png")
fig, ax = plt.subplots(figsize=(8,5))
...
for name, r in rocks.items():
    ...
    metrics.append({
        ...
        "E": p['E_rm_MPa'],
        "c": mc['cohesion_MPa'],
        "phi": mc['friction_angle_deg'],
        "sigma_cm": p['sigma_cm_MPa']
    })
...
ax.set_xticks(x)
ax.set_xticklabels([m['name'] for m in metrics])
ax.set_ylabel("Value")
ax.set_title("Exercise 7: Rock Mass Property Comparison")
ax.legend()
ax.grid(True, axis="y")
save(fig, "07_rock_comparison_bar")

# ========================================================================
# FIGURE 8: Pump — System Curve
# ========================================================================
print("[8] pump_system_curve.png")
fig, ax = plt.subplots(figsize=(8,5))
...
# Pump curve approximation: H = 140 - 0.00008*Q^2
...
ax.legend()
ax.grid(True)
save(fig, "08_pump_system_curve")

# ========================================================================
# FIGURE 9: Slope — FOS vs ru
# ========================================================================
print("[9] slope_fos_ru.png")
fig, ax = plt.subplots(figsize=(8,5))
...
ax.legend()
ax.grid(True)
save(fig, "09_slope_fos_ru")

# ========================================================================
# FIGURE 10: Bench — Cross-section diagram
# ========================================================================
print("[10] bench_cross_section.png")
fig, ax = plt.subplots(figsize=(10, 6))
# Draw bench schematic
bench_h, face_angle, berm = 15, 65, 8
inter_ramp = math.degrees(math.atan(bench_h / (bench_h / math.tan(math.radians(face_angle)) + berm)))

# Bench face
ax.plot([0, bench_h / math.tan(math.radians(face_angle))], [0, bench_h], color="#e94560", lw=3)
# Berm
ax.plot([bench_h / math.tan(math.radians(face_angle)), bench_h / math.tan(math.radians(face_angle)) + berm],
        [bench_h, bench_h], color="#16c79a", lw=3)
# Next bench face
ax.plot([bench_h / math.tan(math.radians(face_angle)) + berm,
         bench_h / math.tan(math.radians(face_angle)) + berm + bench_h / math.tan(math.radians(face_angle))],
        [bench_h, 2*bench_h], color="#e94560", lw=3)
# Overall slope line
overall = math.degrees(math.atan(2*bench_h / (bench_h / math.tan(math.radians(face_angle)) + berm + bench_h / math.tan(math.radians(face_angle)))))
ax.plot([0, bench_h / math.tan(math.radians(face_angle)) + berm + bench_h / math.tan(math.radians(face_angle))],
        [0, 2*bench_h], color="#ffd93d", linestyle="--", lw=2, label=f"Overall slope ≈ {overall:.1f}°")
...
ax.legend()
ax.grid(True)
save(fig, "10_bench_cross_section")

# ========================================================================
# FIGURE 11: Blast Fragmentation — Fragmentation curve
# ========================================================================
print("[11] blast_fragmentation_curve.png")
fig, ax = plt.subplots(figsize=(8,5))
# Simplified fragmentation distribution
sizes = [0.05, 0.1, 0.2, 0.3, 0.5, 0.7, 1.0, 1.5, 2.0]
# Current PF=0.38 (coarser)
pass_current = [10, 25, 45, 60, 75, 85, 92, 97, 99]
# Target PF=0.52 (finer)
...
ax.legend()
ax.grid(True)
save(fig, "11_blast_fragmentation_curve")

# ========================================================================
# FIGURE 12: Subsidence — Profile
# ========================================================================
...
ax.set_xlabel("Distance from Panel Center (m)")
ax.set_ylabel("Subsidence (m)")
ax.set_title("Exercise 12: Longwall Subsidence Profile")
ax.legend()
ax.grid(True)
save(fig, "12_subsidence_profile")

# ========================================================================
# FIGURE 13: Tailings — Cross-section
# ========================================================================
print("[13] tailings_cross_section.png")
fig, ax = plt.subplots(figsize=(10, 6))
# Upstream slope 3H:1V, downstream 2.5H:1V
height = 45
upstream_run = height * 3
downstream_run = height * 2.5
crest = 8
...
ax.legend()
ax.grid(True)
save(fig, "13_tailings_cross_section")

# ========================================================================
# FIGURE 14: Closure — Water balance
# ========================================================================
print("[14] closure_water_balance.png")
fig, ax = plt.subplots(figsize=(8,5))
components = ["Rainfall", "Catchment Runoff", "Groundwater", "Evaporation"]
values = [312000, 234000, 306600, -576000]  # m3/year
...
ax.grid(True, axis="x")
save(fig, "14_closure_water_balance")

# ========================================================================
# FIGURE 15: Feasibility — Cash flow
# ========================================================================
print("[15] feasibility_cash_flow.png")
fig, ax = plt.subplots(figsize=(10, 5))
years = [0, 1, 2, 3, 4, 5]
# Simplified from exercise
cash_flows = [-85, 18.5, 18.5, 18.5, 18.5, 9.2]  # $M
...
ax.grid(True, axis="y")
save(fig, "15_feasibility_cash_flow")
